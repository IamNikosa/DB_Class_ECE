...
CREATE TABLE Buy_Queue (
    ...
    Timestamp DATETIME,
    FOREIGN KEY (Event_ID) REFERENCES Event(ID),
    FOREIGN KEY (Type_ID) REFERENCES Type(Name) ON UPDATE CASCADE, 
...
BEGIN
    ...
    IF v_Visitor_ID IS NOT NULL THEN
        ...
        -- Check if a resale ticket exists for this event and category
        ...
        IF v_Ticket_ID IS NOT NULL THEN
            ...
            INSERT INTO Transaction (ID, Is_Resale, Ticket_ID, Buyer_ID, Payment_ID)
            VALUES (UUID_SHORT(), TRUE, v_Ticket_ID, v_Visitor_ID, NEW.Payment_ID);
...
BEGIN
    ...
    -- Get Type_ID from the ticket to be resold
    SELECT Type_ID INTO v_Type_ID
    FROM Ticket
    WHERE ID = NEW.Ticket_ID;

    -- Find first matching visitor from Buy_Queue for the same event and type
    SELECT BQ.ID, BQ.Payment_ID, VW.Visitor_ID INTO v_Buy_Queue_ID, v_Payment_ID, v_Visitor_ID
    FROM Buy_Queue BQ
    JOIN Visitor_Waitlisted VW ON VW.Buy_Queue_ID = BQ.ID
    JOIN Ticket T ON T.ID = NEW.Ticket_ID
    WHERE BQ.Event_ID = T.Event_ID
      AND BQ.Type_ID = v_Type_ID
    ORDER BY BQ.Timestamp ASC
    LIMIT 1;

    -- If a visitor is found, assign ticket directly instead of inserting into resale
    IF v_Visitor_ID IS NOT NULL THEN

        -- Reassign the spectator
        UPDATE Spectator
        SET Visitor_ID = v_Visitor_ID
        WHERE Ticket_ID = NEW.Ticket_ID;
        ...
        -- Record transaction
        INSERT INTO Transaction (ID, Is_Resale, Ticket_ID, Buyer_ID, Payment_ID)
        VALUES (UUID_SHORT(), TRUE, NEW.Ticket_ID, v_Visitor_ID, v_Payment_ID);

        -- Clean up waitlist and buy queue
        DELETE FROM Visitor_Waitlisted
...
BEGIN
    ...
    UPDATE Ticket
    SET Date_Bought = NOW(), Payment_ID = NEW.Payment_ID
    WHERE ID = NEW.Ticket_ID;
END$$
...
CREATE PROCEDURE handle_visitor_interest(IN p_Visitor_ID INT, IN p_Event_ID INT, IN p_Type_ID INT, IN p_Payment_ID INT)
BEGIN
    DECLARE v_Is_Sold_Out BOOLEAN;
    DECLARE v_New_Ticket_ID INT;
    DECLARE v_Resale_Activated BOOLEAN;
    DECLARE e_Sold_Out BOOLEAN;
    ...
    IF v_Is_Sold_Out THEN
        ...
        -- Check if the resale queue is already activated
        SELECT Activated INTO v_Resale_Activated
        FROM Resale_Queue
        WHERE Event_ID = p_Event_ID;

        -- Activate the resale queue if it's not already
        IF NOT v_Resale_Activated THEN
            CALL activate_resale_if_sold_out(p_Event_ID);
        END IF;

        SELECT Sold_Out INTO e_Sold_Out
        FROM Event
        WHERE Event_ID = p_Event_ID;

        -- Add to buy queue and waitlist
