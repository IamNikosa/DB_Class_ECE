...
FOR EACH ROW
BEGIN
    CALL handle_visitor_interest(NEW.Visitor_ID, NEW.Event_ID);
END$$
...
BEGIN
    ...
    WHERE T.Event_ID = NEW.Event_ID AND T.Type_ID = NEW.Ticket_Category;
    ...
    IF v_Has_Tickets THEN
        ...
        -- Assign ticket to spectator
        INSERT INTO Spectator (Visitor_ID, Ticket_ID)
        VALUES (NEW.Visitor_ID, v_Ticket_ID);
        ...
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Resale ticket assigned to visitor instead of queuing.'
...
CREATE PROCEDURE handle_visitor_interest(IN p_Visitor_ID INT, IN p_Event_ID INT)
BEGIN
    DECLARE v_Is_Sold_Out BOOLEAN;
    ...
    IF v_Is_Sold_Out THEN
        ...
        VALUES ('Standard', p_Event_ID, NOW());
    ...
    ELSE
        ...
        VALUES ('Standard', p_Event_ID, FALSE);
...
-- Procedure: CHECK IF THERE IS TICKET IN RESALE QUEUE (TO BE RESOLD)
DELIMITER $$

CREATE PROCEDURE has_resale_tickets(IN p_Event_ID INT, OUT p_Has_Tickets BOOLEAN)
BEGIN
    DECLARE v_Count INT;

    SELECT COUNT(*) INTO v_Count
    FROM Tickets_In_Resale TIR
    JOIN Ticket T ON TIR.Ticket_ID = T.ID
    WHERE T.Event_ID = p_Event_ID;

    SET p_Has_Tickets = (v_Count > 0);
END$$
