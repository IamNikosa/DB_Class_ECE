...
-- Helps JOIN Festival → Location
CREATE INDEX idx_festival_location_id ON Festival(Location_ID);


------------------------------------------------------ EVENT -----------------------------------------------------

-- Helps JOIN Festival → Event
CREATE INDEX idx_event_festival_id ON Event(Festival_ID);

-- Helps JOIN Stage → Event
CREATE INDEX idx_event_stage_id ON Event(Stage_ID);


-------------------------------------------------- RESALE QUEUE  -----------------------------------------------------

-- Helps JOIN Resale_Queue → Event
CREATE INDEX idx_resale_event_id ON Resale_Queue(Event_ID);


-------------------------------------------------- BUY QUEUE  -----------------------------------------------------

-- Helps JOIN Buy_Queue → Event
CREATE INDEX idx_buy_event_id ON Buy_Queue(Event_ID);

-- Helps JOIN Buy_Queue → Type
CREATE INDEX idx_buy_queue_type_id ON Buy_Queue(Type_ID);

-- Helps JOIN Buy_Queue → Payment
CREATE INDEX idx_buy_queue_payment_id ON Buy_Queue(Payment_ID);


----------------------------------------------------- STAFF -------------------------------------------------------

-- Helps JOIN Security_Staff → event_staff,Staff
CREATE INDEX idx_securitystaff ON Security_Staff(Staff_ID);

-- Helps JOIN Support_Staff → event_staff, Staff
CREATE INDEX idx_supportstaff ON Support_Staff(Staff_ID);

-- Helps JOIN Technical_Staff → event_staff, Staff
CREATE INDEX idx_technicalstaff ON Technical_Staff(Staff_ID);

-- Helps JOIN event_staff → event
CREATE INDEX idx_eventstaff_event ON event_staff(Event_ID);

-- Helps JOIN event_staff → staff
CREATE INDEX idx_eventstaff_staff ON event_staff(Staff_ID);


----------------------------------------------------- TICKET -----------------------------------------------------

-- Helps JOIN Ticket → Event
CREATE INDEX idx_ticket_event ON Ticket(Event_ID);

-- Helps JOIN Ticket → Payment
CREATE INDEX idx_ticket_payment ON Ticket(Payment_ID);

-- Helps JOIN Ticket → Type
CREATE INDEX idx_ticket_type ON Ticket(Type_ID);


---------------------------------------------------- TICKET RESALE -----------------------------------------------

CREATE INDEX idx_tickets_in_resale_event_id ON Tickets_In_Resale(Event_ID);


----------------------------------------------------- SPECTATOR -----------------------------------------------------

-- Helps JOIN Spectator → Ticket
CREATE INDEX idx_spectator_ticket ON Spectator(Ticket_ID);

-- Helps JOIN Spectator → Visitor
CREATE INDEX idx_visitor_ticket ON Spectator(Visitor_ID);


--------------------------------------------------- MEMBERSHIP ----------------------------------------------------

-- Helps JOIN event_membership → band
CREATE INDEX idx_membership_band_id ON Membership(Band_ID);

-- Helps JOIN event_membership → Performer
CREATE INDEX idx_membership_artist_id ON Membership(Artist_ID);


--------------------------------------------- PERFORMER SUBGENRE --------------------------------------------------

-- Helps JOIN Performer_Subgenre → Subgenre
CREATE INDEX idx_performersubgenre_subgenre ON Performer_Subgenre(Subgenre_ID);

-- Helps JOIN Performer_Subgenre → Performer
CREATE INDEX idx_performersubgenre_performer ON Performer_Subgenre(Performer_ID);


-------------------------------------------------- SUBGENRE -----------------------------------------------------

-- Helps JOIN Genre → Subgenre
CREATE INDEX idx_subgenre_genre ON Subgenre(Genre_ID);


------------------------------------------------ VISITOR INTEREST -----------------------------------------------

CREATE INDEX idx_visitor_interested_event_event_id ON Visitor_Interested_Event(Event_ID);
CREATE INDEX idx_visitor_interested_event_visitor_id ON Visitor_Interested_Event(Visitor_ID);
CREATE INDEX idx_visitor_interested_event_type_id ON Visitor_Interested_Event(Type_ID);
CREATE INDEX idx_visitor_interested_event_payment_id ON Visitor_Interested_Event(Payment_ID);


-------------------------------------------------- VISITOR WAITLISTED --------------------------------------------

CREATE INDEX idx_visitor_waitlisted_buy_queue_id ON Visitor_Waitlisted(Buy_Queue_ID);
CREATE INDEX idx_visitor_waitlisted_visitor_id ON Visitor_Waitlisted(Visitor_ID);


-------------------------------------------------- VISITOR SOLD TICKET -------------------------------------------

CREATE INDEX idx_visitor_sold_ticket_seller_id ON Visitor_Sold_Ticket(Seller_ID);
CREATE INDEX idx_visitor_sold_ticket_ticket_id ON Visitor_Sold_Ticket(Ticket_ID);
CREATE INDEX idx_visitor_sold_ticket_transaction_id ON Visitor_Sold_Ticket(Transaction_ID);


---------------------------------------------------- TRANSACTION -------------------------------------------------

-- Helps JOIN Transaction → Payment
CREATE INDEX idx_transaction_payment ON Transaction(Payment_ID);

-- Helps JOIN Ticket → Transaction
CREATE INDEX idx_transaction_ticket ON Transaction(Ticket_ID);

-- Helps JOIN Visitor → Transaction
CREATE INDEX idx_visitor_payment ON Transaction(Buyer_ID);


------------------------------------------------ PERFORMANCE ----------------------------------------------------

-- Helps JOIN Performance → Performer
CREATE INDEX idx_performance_performer ON Performance(Performer_ID);

-- Helps JOIN Performance → Event
CREATE INDEX idx_performance_event ON Performance(Event_ID);


------------------------------------------------------ REVIEW ----------------------------------------------------

-- Helps JOIN Review → Ticket
CREATE INDEX idx_review_ticket ON Review(Ticket_ID);


------------------------------------------------------- IMAGES ---------------------------------------------------

CREATE INDEX idx_performer_image_performer_id ON Performer_Image(Performer_ID);
CREATE INDEX idx_performer_image_image_id ON Performer_Image(Image_ID);

CREATE INDEX idx_stage_image_stage_id ON Stage_Image(Stage_ID);
CREATE INDEX idx_stage_image_image_id ON Stage_Image(Image_ID);

CREATE INDEX idx_staff_image_staff_id ON Staff_Image(Staff_ID);
CREATE INDEX idx_staff_image_image_id ON Staff_Image(Image_ID);

CREATE INDEX idx_event_image_event_id ON Event_Image(Event_ID);
CREATE INDEX idx_event_image_image_id ON Event_Image(Image_ID);
