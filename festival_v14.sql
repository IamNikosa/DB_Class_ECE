...
-- Trigger: AFTER INSERT ON TICKETS IN RESALE 1
DELIMITER $$

CREATE TRIGGER trg_before_insert_ticket_resale
AFTER INSERT ON Tickets_In_Resale
FOR EACH ROW
BEGIN
    ...
    -- Get Type_ID and Seller_ID from the ticket
    ...
    -- Find the first matching visitor in the buy queue
    ...
    -- If a match is found, assign the ticket directly
    IF v_Visitor_ID IS NOT NULL THEN
        ...
        SET v_Transaction_ID = UUID_SHORT(); -- You could also use another way to generate transaction IDs
        INSERT INTO Transaction (ID, Is_Resale, Ticket_ID, Buyer_ID, Payment_ID)
        VALUES (v_Transaction_ID, TRUE, NEW.Ticket_ID, v_Visitor_ID, v_Payment_ID);

        -- Record the sale
        INSERT INTO Visitor_Sold_Ticket (Seller_ID, Ticket_ID, Transaction_ID)
        VALUES (v_Seller_ID, NEW.Ticket_ID, v_Transaction_ID);

        -- Clean up the queues (remove visitor from waitlist and buy queue)
        DELETE FROM Visitor_Waitlisted
        ...
        -- Prevent insertion into the Tickets_In_Resale table (abort insert)
...
-- Trigger: AFTER INSERT TICKET IN RESALE 2
DELIMITER $$

CREATE TRIGGER trg_after_insert_ticket_resale
AFTER INSERT ON Tickets_In_Resale
FOR EACH ROW
BEGIN
    ...
    -- Check if resale queue for the event already exists
    SELECT COUNT(*) INTO v_Exists
    FROM Resale_Queue
    WHERE Event_ID = v_Event_ID;

    -- If not exists, insert one with proper activation depending on sold-out status
    IF v_Exists = 0 THEN
