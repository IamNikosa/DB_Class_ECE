...
-- Trigger to ensure reviews are only submitted for activated and owned tickets
DELIMITER $$
...
CREATE PROCEDURE Register_Buyer(IN p_Event_ID INT, IN p_Category VARCHAR(100))
BEGIN
...
BEGIN
    ...
    read_loop: LOOP
        ...
        IF v_Buyer_ID IS NOT NULL THEN
            DELETE FROM Spectator WHERE Ticket_ID = v_Ticket_ID;
            INSERT INTO Spectator (Visitor_ID, Ticket_ID) VALUES (v_Buyer_ID, v_Ticket_ID);
            ...
            DELETE FROM Visitor_Waitlisted WHERE Buy_Queue_ID = v_Buy_Queue_ID AND Visitor_ID = v_Buyer_ID;
